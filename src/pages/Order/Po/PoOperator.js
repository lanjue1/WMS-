...
export default class PoDetail extends Component {
	...
	componentDidMount() {
		...
		this.setState({
			...
			this.getDetail()
			this.getDetailList({ type: 'poDetailList' })
		});
	...
	handleSearch = formValues => {
		...
		this.getDetailList({ value, type: 'poDetailList' })
	...
	saveInfo = () => {
		const { poDetails = {}, dispatch } = this.props
		...
		const { billTypeId=[], actualAmount, forwarder = [], loadingTime, arrivedTime, declareTime, declarationCompletionTime, bizDate, estimateReceiveTime, realReceiveTimeStart, realReceiveTimeEnd } = newParams

		// 处理远程下拉
		// customerId.length > 0 ? newParams.customerId = customerId[0].id : newParams.customerId = undefined
		forwarder.length > 0 ? newParams.forwarder = forwarder[0].id : newParams.forwarder = undefined
		billTypeId.length > 0 ? newParams.billTypeId = billTypeId[0].code : newParams.billTypeId = undefined
		// 处理时间
		...
		//  处理附件
		const _fileToken = poDetails[detailId] && poDetails[detailId].fileToken || []
		const _signForm = poDetails[detailId] && poDetails[detailId].signForm || []
		...
		dispatch({
			...
			callback: (data) => {
				if (detailId) {
					...
					router.push(`/inboundManagement/PoDetail/edit/${data.data}`);
				}
			}
		})
	}
	...
	extraCur = () => {
		return <div onClick={(event) => event.stopPropagation()} style={{ display: 'flex' }} >
			<label style={{ width: '80px' }}>货值币种</label>
	...
	render() {
		const { detail, poDetails, match: { params }, form, poDetailList
		} = this.props
		const {
			disabled,
			expandForm,
		...
		let selectDetails = currentId && poDetails[currentId] || {};
		const commonParams = { getFieldDecorator: form.getFieldDecorator }
		const firstFormItem = (
			<AntdFormItem label={'明细单号'} code="bolNo" {...commonParams}>
		...
		const otherFormItem = [
			...
			[
				<AntdFormItem label={'发票号'} code="billingNo" {...commonParams}>
				...
				<AntdFormItem label={'供应商料号'} code="billingNo" {...commonParams}>
					<div style={{ position: 'relative', width: '100%' }}><AntdInput zIndex={998} type='textarea' mode='search' /></div>
				</AntdFormItem>,
			],
		...
		const partsButtons = (<Button.Group>
			<AdButton text={'删除'} onClick={() => this.ableOperate('poDetailDelete')} />
			<AdButton text={'添加'} onClick={() => this.ableOperate('poDetailAdd')} />
		</Button.Group>)
		const panelValue = [
			{ title: '基础信息' },
			{ title: '货物信息' },
		...
		return (
			<div className={styles.CollapseUpdate}>
				<PageHeaderWrapper title={genExtraBasicInfo()} >
						...
						<Panel header={panelValue[0].title} key='1' style={customPanelStyle} >
							<div className={styles.tableListForm}>
								<Tabs defaultActiveKey='1' onChange={this.tabsChange} tabBarStyle={{ marginBottom: '10px' }}>
									...
									<TabPane forceRender tab={firstTabs[1].title} key='2'>
					...
					{visible && <AddDetailsModal detail={selectDetails} refreshList={() => { this.getDetailList({ type: 'poDetailList' }); this.getDetail() }} selectDetails={selectModalDetail} onCancel={this.onCancel} visible={visible} />}
				</PageHeaderWrapper>
			</div>
		)
	}
