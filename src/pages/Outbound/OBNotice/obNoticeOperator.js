...
export default class obNoticeDetail extends Component {
	...
	render() {
		...
		const otherFormItem = [
			...
			[
				<AntdFormItem label={'发票号'} code="invoiceNo" {...commonParams}>
		...
		const partsButtons = (<Button.Group>
			<AdButton text={'删除'} onClick={() => this.ableOperate('detailDelete')} />
			<AdButton text={'添加'} onClick={() => this.ableOperate('detailAdd')} />
		</Button.Group>)
		const panelValue = [
			{ title: '基础信息' },
			{ title: '货物信息' },
